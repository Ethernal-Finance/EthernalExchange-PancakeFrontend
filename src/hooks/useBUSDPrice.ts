...
export const useethfinBusdPrice = (): Price | undefined => {
  const ethfinBusdPrice = useBUSDPrice(tokens.ethfin)
  return ethfinBusdPrice
}
...
export const useBUSDCakeAmount = (amount: number): number | undefined => {
  const ethfinBusdPrice = useethfinBusdPrice()
  if (ethfinBusdPrice) {
    return multiplyPriceByAmount(ethfinBusdPrice, amount)
  }
  return undefined
}
